...
int write_block(uint32_t block_no, uint8_t *data, bool quiet, WriteState *state) {
    ...
    if ((bl->flags & UF2_FLAG_NOFLASH) || bl->payloadSize > 256 || (bl->targetAddr & 0xff) ||
        ...
        if ( first_write ) {
            memset(state->hash, 0, 32);
            first_write = false;
            led_state(STATE_WRITING_STARTED);
        }
        
        uint8_t hash[32];
        cc310_sha256(data, 32+256, hash);
        for (uint8_t i=0; i<sizeof(hash); i++) state->hash[i] ^= hash[i];
        if (bl->blockNo == bl->numBlocks-2) memcpy(state->signature, bl->data+256, 128);
        if (bl->blockNo == bl->numBlocks-1) memcpy(state->signature+128, bl->data+256, 128);
