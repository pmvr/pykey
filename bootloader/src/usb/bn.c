...
void bignum_from_bytes(struct bn* n, const uint8_t *b, int len_b)
{
    // assume that byte array b is in big endian
#if (WORD_SIZE == 1)
    for (int i=0; i<len_b; i++) n->array[i] = b[len_b-1-i];
#elif (WORD_SIZE == 2)
    for (int i=0; i<len_b; i+=2) n->array[i/2] = b[len_b-2-i]*256 + b[len_b-1-i];
#elif (WORD_SIZE == 4)
    for (int i=0; i<len_b; i+=4) n->array[i/4] = b[len_b-4-i]*(256*256*256) + b[len_b-3-i]*(256*256) + b[len_b-2-i]*256 +  b[len_b-1-i];
#endif    
}


void bignum_to_bytes(struct bn* n, uint8_t *b, int len_b)
{
    // assume that byte array b is in big endian
#if (WORD_SIZE == 1)
    for (int i=0; i<len_b; i++) b[len_b-1-i] = n->array[i];
#elif (WORD_SIZE == 2)
    for (int i=0; i<len_b; i+=2) {
        b[len_b-1-i] = (uint8_t)(n->array[i/2]);
        b[len_b-2-i] = (uint8_t)(n->array[i/2] >> 8);
    }
#elif (WORD_SIZE == 4)
    for (int i=0; i<len_b; i+=4) {
        b[len_b-1-i] = (uint8_t)(n->array[i/4]);
        b[len_b-2-i] = (uint8_t)(n->array[i/4] >> 8);
        b[len_b-3-i] = (uint8_t)(n->array[i/4] >> 16);
        b[len_b-4-i] = (uint8_t)(n->array[i/4] >> 24);
    }
#endif    
}
